...
export function GlobalAiAssistant() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'ai', text: 'Chào thầy/cô! Em là Trợ lý điều hành AI của Trạm Điều Hành. Thầy/cô cần hỏi gì?' },
  ]);
  ...
  async function send(text: string) {
    ...
    try {
      const resp = await fetch('/api/gemini', {
        ...
        body: JSON.stringify({ mode: 'ops-chat', message: q }),
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40" onClick={closeAssistant}>
      ...
      >
        <div className="bg-hoa-950 text-white px-4 py-3 flex items-center gap-2 shrink-0">
          <Bot size={18} className="text-gold-400" />
          <div className="flex-1">
            <p className="text-sm font-bold leading-tight">Trợ lý điều hành AI</p>
