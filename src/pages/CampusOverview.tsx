...
export function CampusOverview() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-xs font-semibold tracking-wide text-gold-500">01 NHÀ TRƯỜNG · 04 ĐIỂM TRƯỜNG</p>
        <h2 className="text-2xl font-bold text-ink mt-0.5">4 điểm trường</h2>
      </div>

      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
        {CAMPUSES.map((c) => {
          const head = LEADERSHIP.find((l) => l.campusId === c.id);
          const cbgvnv = c.teacherCount !== undefined && c.staffCount !== undefined ? c.teacherCount + c.staffCount : null;
          return (
            <div key={c.id} className="rounded-xl border border-black/10 bg-white p-5 flex flex-col">
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                <div className="text-center">
                  ...
                  <p className="text-sm font-semibold text-ink mt-1">{cbgvnv ?? 'Chưa cập nhật'}</p>
              ...
              <Link
                ...
                className="mt-5 block rounded-lg border border-hoa-700 text-hoa-700 text-sm font-medium py-2 text-center hover:bg-hoa-950 hover:text-white hover:border-hoa-950 transition-colors"
              >
