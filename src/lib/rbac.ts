...
// Nhóm BGH (Hiệu trưởng, Phó Hiệu trưởng, CNTT, Tổ trưởng Văn phòng) dùng
// chung 1 mã (qt2026) và có TOÀN QUYỀN xem/tải lên/cài đặt/chỉnh sửa mọi
// module trong hệ thống — không phân biệt granular như trước nữa.
const FULL_ACCESS: Partial<Record<ModuleKey, Permission[]>> = {
  tong_quan: ['view', 'create', 'edit', 'delete', 'export', 'report'],
...
const MATRIX: Partial<Record<RoleId, Partial<Record<ModuleKey, Permission[]>>>> = {
  ...
  hieu_truong: FULL_ACCESS,
  pho_hieu_truong: FULL_ACCESS,
  cntt: FULL_ACCESS,
  to_truong_vp: FULL_ACCESS,
  bi_thu_dang: {
    tong_quan: ['view'],
  ...
  // khu vực nhạy cảm (Quản trị, Công tác Đảng, Phân tích và dự báo) chỉ cho
  // XEM, không được tạo/sửa. Nới lại khi hệ thống hết giai đoạn thử nghiệm.
  giao_vien: {
    tong_quan: ['view'],
    quan_tri: ['view'],
    cong_tac_dang: ['view'],
